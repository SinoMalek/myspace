	...
	if ($mainmenu == 'project') {
		if (isModEnabled('projet')) {
			...
			if (empty($conf->global->PROJECT_HIDE_TASKS)) {
				...
				/////Khalil modification
				$newmenu->add("/projet/ganttview.php?id=open", "Vue Gant", 1, $user->rights->projet->lire);
			}
		}
	}
}
...
function get_left_menu_hrm($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
	...
	if ($mainmenu == 'hrm') {
        ...
        if (isModEnabled('holiday')) {
            ...
            $newmenu->add("/holiday/autorization/list.php?mainmenu=hrm&leftmenu=holiday", $langs->trans("ATTitreMenu"), 0, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday', 0, '', '', '', img_picto('', 'holiday', 'class="paddingright pictofixedwidth"'));
            $newmenu->add("/holiday/autorization/card.php?mainmenu=hrm&leftmenu=holiday&action=create", $langs->trans("New"), 1, $user->hasRight('holiday',  'write'), '', $mainmenu);
            //$newmenu->add("/holiday/card_group.php?mainmenu=hrm&leftmenu=holiday&action=create", $langs->trans("NewHolidayForGroup"), 1, ($user->hasRight('holiday',  'writeall') && $user->hasRight('holiday',  'readall')), '', $mainmenu, 'holiday_sm');
            $newmenu->add("/holiday/autorization/list.php?mainmenu=hrm&leftmenu=holiday", $langs->trans("List"), 1, $user->hasRight('holiday',  'read'), '', $mainmenu);
            if ($usemenuhider || empty($leftmenu) || $leftmenu == "holiday") {
                $newmenu->add("/holiday/autorization/list.php?search_status=1&mainmenu=hrm&leftmenu=holiday", $langs->trans("DraftCP"), 2, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
                $newmenu->add("/holiday/autorization/list.php?search_status=2&mainmenu=hrm&leftmenu=holiday", $langs->trans("ToReviewCP"), 2, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
                $newmenu->add("/holiday/autorization/list.php?search_status=3&mainmenu=hrm&leftmenu=holiday", $langs->trans("ApprovedCP"), 2, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
                $newmenu->add("/holiday/autorization/list.php?search_status=4&mainmenu=hrm&leftmenu=holiday", $langs->trans("CancelCP"), 2, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
                $newmenu->add("/holiday/autorization/list.php?search_status=5&mainmenu=hrm&leftmenu=holiday", $langs->trans("RefuseCP"), 2, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
            }
            //$newmenu->add("/holiday/define_holiday.php?mainmenu=hrm", $langs->trans("NewHolidayForGroup"), 1, $user->hasRight('holiday',  'read'), '', $mainmenu, 'holiday_sm');
            $newmenu->add("/holiday/month_report.php?mainmenu=hrm&leftmenu=holiday", $langs->trans("MenuReportMonth"), 1, $user->hasRight('holiday',  'readall'), '', $mainmenu, 'holiday_sm');
            $newmenu->add("/holiday/view_log.php?mainmenu=hrm&leftmenu=holiday", $langs->trans("MenuLogCP"), 1, $user->hasRight('holiday',  'define_holiday'), '', $mainmenu, 'holiday_sm');
        }
