...
class Holiday extends CommonObject
{
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        $sql .= " ".((int) $user->id).",";
        $sql .= " ".((int) $conf->entity).","; // Added comma here
	 ...
	 */
	 //khalil
// Ajoutez cette méthode à votre classe Holiday
public function getTypesById($typeId) {
    $sql = "SELECT * FROM ".MAIN_DB_PREFIX."c_hrm_leave_type";
	...
	public function nb_days_requestedtot($start_date, $end_date,$halfday) {
		...
		$end = $end_date;
		$count = 0;
		dol_syslog("---------------------------halfdaaaay " . $halfday, LOG_DEBUG);
		...
		// Ajouter un log pour le résultat final
		dol_syslog("---------------------------Total number of days: " . $count, LOG_DEBUG);
	 if ($halfday <> '0') {
				$count = $count-0.5; // Ajoutez 0.5 pour chaque demi-journée
				dol_syslog("---------------------------Total number of days demis: " . $count, LOG_DEBUG);
	 }
...
public function getTotalHollyDaysCount($start_date, $end_date) {
    ...
    while ($start <= $end) {
        ...
        if ($dayOfWeek >= 6) { // 6 et 7 représentent le samedi et le dimanche
            $count++;
        }

        // Vérifier si le jour est un jour férié
        $formattedDate = date("Y-m-d", $start);
        $sql = 'SELECT COUNT(day) AS count FROM '.MAIN_DB_PREFIX.'c_hrm_public_holiday';
        $sql .= ' WHERE (day = '.date('j', $start).' AND month = '.date('n', $start).' AND year = 0)';
        $sql .= ' OR (day = '.date('j', $start).' AND month = '.date('n', $start).' AND year = '.date('Y', $start).')';
    ...
    $future_balance = $balance - $nb_days_requested;

    dol_syslog("------------Validation process started for leave request ID: ".$this->id, LOG_DEBUG);
	...
	public function update($user = null, $notrigger = 0)
	{
		...
		if ($checkBalance > 0 && $this->statut != self::STATUS_DRAFT) {
			...
			if ($balance < 0) {
				$this->error = 'LeaveRequestCreationBlockedBecauseBalancewillbeNegative';
				return -1;
			}
		}
